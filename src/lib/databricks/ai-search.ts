...
import { sanitizeExternalQuery } from "@/lib/privacy";
export async function searchDevelopmentMemory(query: string): Promise<{ evidence: Evidence[]; status?: string }> {
  const safeQuery = sanitizeExternalQuery(query);
  if (!safeQuery.value) return { evidence: [], status: safeQuery.status };
  ...
  try {
    const result = await databricksFetch(`/api/2.0/vector-search/indexes/${encodeURIComponent(config.searchEndpoint)}/query`, { method: "POST", body: JSON.stringify({ query_text: safeQuery.value, num_results: 5 }) }) as Record<string, unknown>;
